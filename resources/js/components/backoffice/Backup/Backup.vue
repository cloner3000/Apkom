        ...
        <div class="card">
          <div class="card-body table-responsive p-0">
            <table class="table table-hover">
              <tbody><tr>
                <th>No</th>
                <th>Nama Pencadangan</th>
                <th>Ukuran</th>
                <th>Tanggal Pembuatan</th>
                <th width="20%" class="text-center">Aksi</th>
              </tr>
              <tr v-for="(data, index) in backups.data" :key="index">
                <td>{{index+1}}</td>
                <td>{{data.name}}</td>
                <td>{{data.size}} kb</td>
                <td>{{data.created}}</td>
                <td class="text-center">
                    <button @click="restoreBackup(data.name)" class="btn btn-warning btn-sm"><i  class="fas fa-redo-alt text-white"></i></button>
                    <button @click="downloadBackup(data.name)" class="btn btn-primary btn-sm"><i  class="fas fa-download"></i></button>
                    <button @click="deleteBackup(data.name)" class="btn btn-danger btn-sm"><i  class="fas fa-trash"></i></button>
                </td>
              </tr>
            </tbody></table>
          </div>
...
<script>
    export default {
        ...
        methods:{
          getBackups() {
            if(this.$gate.isWarek()){
              ...
              this.$Progress.fail();
                toast.fire({
                  type: 'error',
                  title: 'Gagal memuat data pencadangan'
                });
              });
            }
          },
          createBackup(){
            this.$Progress.start();
            this.form.post('api/backup')
            .then(() => {
              cusEvent.$emit('ReloadData');
              toast.fire({
                type: 'success',
                title: 'Berhasil melakukan pencadangan'
              });
              this.form.reset();
              this.$Progress.finish();
            })
            .catch(() => {
              ...
              toast.fire({
                type: 'error',
                title: 'Gagal melakukan pencadangan'
              });
            });
          },
          restoreBackup(name){
              ...
              if (result.value){
                ...
                .catch(() => {
                  ...
                  swal.fire({
                    type: 'error',
                    title: 'Oops...',
                    text: 'Gagal mengembalikan basis data'
                  });
                });
              }
            });
          ...
          downloadBackup(name){
            ...
            .catch(() => {
              ...
              toast.fire({
              type: 'error',
              title: 'Gagal mencadangkan basis data'
              });
            })
          },
          deleteBackup(name){
            swal.fire({
              ...
              if (result.value){
                this.form.delete('api/backup/'+name)
                .then(() => {
                  this.$Progress.start();
                  swal.fire(
                    'Deleted!',
                ...
                .catch(() => {
                  ...
                  swal.fire({
                    type: 'error',
                    title: 'Oops...',
                    text: 'Pencadangan gagal dihapus'
                  });
                });
              }
            });
